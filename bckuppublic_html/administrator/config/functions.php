     ...
     function info_upload_image_only($f,$dest_dir,$newname) // file upload , check formats,check
      {
	   ...
	   // Image Resize
    function Image_Width_fixed_Height_Resize($filename,$newwidth,$newheight,$dir,$thumdir) // this function wi  resize ony jpg images 1-eatured,0-norma
      {
	    copy($dir."/".$filename,$thumdir."/".$filename);
        $x=$filename;
        $filename12=$thumdir."/".$filename;
        $extnarr=explode(".",$x);
        // Get new dimensions
        ...
        if($width > $newwidth)
		{
			$new_height=Ceil(($newwidth * $height)/$width);
			$new_width = $newwidth;
		}else{
            $new_height=Ceil(($newwidth * $height)/$width);
			$new_width = $newwidth;
     ...
     // Image Resize
    function Image_Width_Height_Resize($filename,$newwidth,$newheight,$dir,$thumdir) // this function wi  resize ony jpg images 1-eatured,0-norma
      {
        copy($dir."/".$filename,$thumdir."/".$filename);
        $x=$filename;
       $filename12=$thumdir."/".$filename;
...
// Redirect to another page or site
    function tep_redirect($url)
	 ...
	 function sel_droupdown()
     {
       $pagename=array('amenities.php','accomdation.php','accomdation_detatils.php.php','booking.php','clients.php','contact.php','location.php','testimonials.php');
       return $pagename;
     }
?>