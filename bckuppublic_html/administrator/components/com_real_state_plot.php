...
class LoadPageContentClass
 {
   function ShowPageContent()
    {
      ...
      if(isset($_REQUEST['Action']) && $_REQUEST['Action'] == "ActionSaveNewData" && $PageAction =="SaveNewData")
       {  
         $SqlOrder = "SELECT MAX(newOrder) AS MaxOrder FROM ".TBL_REAL_STATE_PLOT.";";
         $RowOrder = mysql_fetch_object(mysql_query($SqlOrder));
         $OrderNo = $RowOrder->MaxOrder + 1;
         $SqlMaxID = "SELECT MAX(plot_id) AS MaxID FROM ".TBL_REAL_STATE_PLOT.";";
         $RowMaxID = mysql_fetch_object(mysql_query($SqlMaxID));
         $MaxID = $RowMaxID->MaxID + 1;
		  extract($_POST);

         $SqlInsert = "INSERT INTO ".TBL_REAL_STATE_PLOT." (plot_id, plot, publish, newOrder, 
		 updtime)
         VALUES (".$MaxID.",".tosql($_POST['plot'], "Text").",
      ...
      if(isset($_REQUEST['Action']) && $_REQUEST['Action'] == "ActionSaveData" && $PageAction =="SaveData")
       {   
		...
		$SqlUpdate = "UPDATE ".TBL_REAL_STATE_PLOT." SET
		 plot=".tosql($_POST['plot'], "Text")."
		 WHERE plot_id='".$DataID."';";
         //die($SqlUpdate);
      ...
      if(isset($_REQUEST['Action']) && $_REQUEST['Action'] == "ActionSortUp" && $PageAction =="SortUp")
       {
         $SqlOrder = "SELECT newOrder FROM ".TBL_REAL_STATE_PLOT." WHERE plot_id='".$DataID."';";
         $RowOrder = mysql_fetch_object(mysql_query($SqlOrder));
         $OrderNo = $RowOrder->newOrder - 1;
         $SqlUpdate01 = "UPDATE ".TBL_REAL_STATE_PLOT." SET newOrder = newOrder + 1 WHERE newOrder='".$OrderNo."';";
         mysql_query($SqlUpdate01);
         $SqlUpdate02="UPDATE ".TBL_REAL_STATE_PLOT." SET newOrder = newOrder - 1 WHERE plot_id='".$DataID."';";
      ...
      if(isset($_REQUEST['Action']) && $_REQUEST['Action'] == "ActionEnableRecord" && $PageAction =="EnableRecord")
       {
         $SqlUpdate01 = "UPDATE ".TBL_REAL_STATE_PLOT." SET publish='1' WHERE plot_id='".$DataID."';";
         mysql_query($SqlUpdate01);
         tep_redirect(tep_href_link($ActionPath));
       }                 
      ...
      if(isset($_REQUEST['Action']) && $_REQUEST['Action'] == "ActionDisableRecord" && $PageAction =="DisableRecord")
       {
         $SqlUpdate01 = "UPDATE ".TBL_REAL_STATE_PLOT." SET publish='0' WHERE plot_id='".$DataID."';";
         mysql_query($SqlUpdate01);
         tep_redirect(tep_href_link($ActionPath));
       }                 
      ...
      if(isset($_REQUEST['Action']) && $_REQUEST['Action'] == "ActEnableAllRecord" && $PageAction =="EnableAllRecord")
       {
         ...
         for($i=0;$i < $Count; $i++)
          {     
             $SqlUpdate01 = "UPDATE ".TBL_REAL_STATE_PLOT." SET publish='1' WHERE plot_id='".$checkitem[$i]."';";
            mysql_query($SqlUpdate01);
          }
         tep_redirect(tep_href_link($ActionPath));
       }
      ...
      if(isset($_REQUEST['Action']) && $_REQUEST['Action'] == "ActDisableAllRecord" && $PageAction =="DisableAllRecord")
       {
         ...
         for($i=0;$i < $Count; $i++)
          {     
           $SqlUpdate01 = "UPDATE ".TBL_REAL_STATE_PLOT." SET publish='0' WHERE plot_id='".$checkitem[$i]."';";
            mysql_query($SqlUpdate01);
          }               
         tep_redirect(tep_href_link($ActionPath));
       }
      ...
      if(isset($_REQUEST['Action']) && $_REQUEST['Action'] == "ActionDelete" && $PageAction =="Delete")
       {
         ...
         for($i=0;$i < $Count; $i++)
          { 
			 $ima_file=mysql_fetch_object(mysql_query("SELECT feed_image FROM ".TBL_REAL_STATE_PLOT." WHERE  plot_id='".$checkitem[$i]."'"));
			 @unlink('../feedback_img/'.$ima_file->feed_image);
			 @unlink('../feedback_imgthumb/'.$ima_file->feed_image);
			 $SqlDelete = "DELETE FROM ".TBL_REAL_STATE_PLOT." WHERE plot_id='".$checkitem[$i]."';";
            ...
            <tr>
              ...
              <table width="100%" border="0" cellspacing="2" cellpadding="2" >
				...
				<tr>
				  ...
				  <input type="text" name="plot" id="plot" value="" size="30" style="width:57%;" class="input_box" >
				  </td>
                </tr>
   ...
   function EditArticlesRecords($CfgGroupID,$DataID)
    {
      $SqlEditRecord="SELECT * FROM ".TBL_REAL_STATE_PLOT." WHERE  plot_id='".$DataID."';";
            ...
            <tr>
              ...
              <table width="100%" border="0" cellspacing="2" cellpadding="2" >
				...
				<tr>
                  ...
                  <td class="label-text" align="center" style="width:10px">:</td>
                  <td class="label-text" align="left" style="width:70%">
				   <input type="text" name="plot" id="plot" value="'.$RowEditRecord->plot.'" size="30" style="width:57%;" class="input_box" >
				  </td>
                </tr>
   ...
   function ViewArticlesRecords($CfgGroupID,$DataID)
    {
      $SqlViewRecord="SELECT * FROM ".TBL_REAL_STATE_PLOT." WHERE  plot_id='".$DataID."';";
   ...
   function ViewNewsRecords($CfgGroupID)
    {
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        $SqlTotCount = "SELECT COUNT(plot_id) AS TotalRecord FROM ".TBL_REAL_STATE_PLOT.";";
...
<!-- js and css code -->		   
           <tr>
             ...
             <table id="table-1" width="100%" border="0" cellspacing="0" cellpadding="0" >
               ...
               $SqlOrder = "SELECT MAX(newOrder) AS MaxOrder , MIN(newOrder) AS MinOrder FROM ".TBL_REAL_STATE_PLOT.";";
               $RowOrder = mysql_fetch_object(mysql_query($SqlOrder));
               $SqlRecord = "SELECT * FROM ".TBL_REAL_STATE_PLOT." ORDER BY newOrder ASC ".$PageLimit.";";
               ...
               while($RowRecord=mysql_fetch_object($RstRecord))
                {  
                  ...
                  <tr class="text_tr" id="'.$SlNo.'">
                    <td><input type="checkbox" name="checkitem[]" id="checkitem" onclick="CheckIndividual(this);" value="'.$RowRecord->plot_id.'" title="'.CONSTANT_CHECKBOX_TITLE_CAPTION.'" ></td>
                    <td align="center">'.$SlNo.'</td>
					<td>'.$RowRecord->plot.'</td>
					
					<td align="center"><a href="javascript: SingleFuncViewRecord('.$RowRecord->plot_id.');" title="'.CONSTANT_BUTTON_TEXT_VIEW.'"><img title="'.CONSTANT_BUTTON_TEXT_VIEW.'" src="images/button/articles/view.png" border="0" /></a></td>
                   <td align="center"><a href="javascript: SingleFuncEditRecord('.$RowRecord->plot_id.');" title="'.CONSTANT_BUTTON_TEXT_EDIT.'"><img title="'.CONSTANT_BUTTON_TEXT_EDIT.'" src="images/button/articles/edit.png" border="0" /></a></td>
                    <td align="center">
                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        ...
                        if($RowRecord->newOrder != $RowOrder->MinOrder)
                         {
                           $fcontent .= '<a href="javascript: FuncRecordSortUp('.$RowRecord->plot_id.');" title="'.CONSTANT_BUTTON_TEXT_SORT_UP.'" ><img src="images/button/articles/up_sml.png" border="0" title="'.CONSTANT_BUTTON_TEXT_SORT_UP.'" ></a>';
                         }
                        else
                         {
                           $fcontent .= "&nbsp;";
                        ...
                        if($RowRecord->newOrder != $RowOrder->MaxOrder)
                         {
                           $fcontent .= '<a href="javascript: FuncRecordSortDown('.$RowRecord->plot_id.');" title="'.CONSTANT_BUTTON_TEXT_SORT_DOWN.'" ><img src="images/button/articles/down_sml.png" border="0" title="'.CONSTANT_BUTTON_TEXT_SORT_DOWN.'"></a>';
                         }
                        else
                         {
                           $fcontent .= "&nbsp;";
                    ...
                    if($RowRecord->publish == "1")
                     {
                       $fcontent .= '<a href="javascript: FuncDisableRecord('.$RowRecord->plot_id.');" title="'.CONSTANT_BUTTON_TEXT_ENABLE.'"><img src="images/button/articles/enable-sml.png" title="'.CONSTANT_BUTTON_TEXT_ENABLE.'" border="0" ></a>';
                     }
                    else
                     {
                       $fcontent .= '<a href="javascript: FuncEnableRecord('.$RowRecord->plot_id.');" title="'.CONSTANT_BUTTON_TEXT_DISABLE.'"><img src="images/button/articles/disable-sml.png" title="'.CONSTANT_BUTTON_TEXT_DISABLE.'" border="0" ></a>';
                     }
                    $fcontent .= '</td>  
                    <!--td align="right">'.$RowRecord->plot_id.'</td-->
                  </tr>';
                  $SlNo++;
                }
               $fcontent .= '
