<?php                          
     //--------------------------------------
     // Select Modules for the page
     //--------------------------------------
     function FuncSelectPageModule($PageID = SB_PAGE_INDEX, $ModID = "")
      {
        $SqlCheck = "SELECT MenuID FROM `sb_module_to_menu` WHERE ModID= '".$ModID."' AND MenuID='".$PageID."';";  
        //echo mysql_num_rows(mysql_query($SqlCheck)).$SqlCheck;
        //die;
     ...
     //--------------------------------------
     // Trim (Find only Text from HTML Page )
     //--------------------------------------
     function FuncTrimHTMLPage($html)
      {
        $Arr1 = explode("<",$html);
        $Str = '';
        $i=0;
